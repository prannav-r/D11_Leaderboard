...
def get_user_stats(user_id: int) -> List[Tuple[int, bool, List[Tuple[int, str, str]]]]:
    ...
    try:
        ...
        alert_preference = get_user_alert_preference(user_id)
        
        # Get recent match wins
        match_wins = get_user_match_wins(user_id)
...
def get_user_alert_preference(user_id: int) -> bool:
    ...
    try:
        response = supabase.table('points').select('alert_enabled').eq('username', f'<@{user_id}>').execute()
        if not response.data:
            return False
...
def get_users_with_alerts() -> List[int]:
    ...
    try:
        response = supabase.table('points').select('username').eq('alert_enabled', True).execute()
        if not response.data:
            return []
...
def has_used_win_today(match_number: int) -> bool:
    ...
    try:
        ...
        response = supabase.table('history').select('*').eq('match_number', match_number).execute()
        if not response.data:
            return False
